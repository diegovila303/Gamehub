          ...
          {activeChat.avatar ? (
            ...
            <div className="w-8 h-8 rounded-full bg-secondary flex items-center justify-center">
              {isGroup
                ...
                : <span className="text-sm font-bold text-muted-foreground">{(activeChat.title || '?')[0].toUpperCase()}</span>}
            </div>
          )}
          <div>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide px-4 py-4">
          ...
          {messages.map((msg) => (
            <ChatBubble key={msg.id}
              message={{ ...msg, content: msg.text, created_date: msg.createdAt }}
              isMine={msg.from === user?.uid}
              senderName={isGroup && msg.from !== user?.uid ? msg.fromName : null} />
          ))}
          <div ref={bottomRef} />
        </div>
    ...
    <div className="p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => setShowNewGroup(!showNewGroup)}
          className="w-9 h-9 rounded-xl bg-card border border-border flex items-center justify-center text-muted-foreground hover:text-primary hover:border-primary/30 transition-all">
      ...
      <div className="flex gap-2 mb-4">
        <button onClick={() => setTab('chats')}
          className={`flex-1 py-2 rounded-xl text-sm font-medium transition-all ${tab === 'chats' ? 'bg-primary/15 text-primary' : 'text-muted-foreground hover:bg-secondary'}`}>
          Conversaciones
        </button>
        <button onClick={() => setTab('friends')}
          className={`flex-1 py-2 rounded-xl text-sm font-medium transition-all ${tab === 'friends' ? 'bg-primary/15 text-primary' : 'text-muted-foreground hover:bg-secondary'}`}>
          Amigos
        </button>
      </div>
        ...
        <div className="mb-4 p-4 rounded-xl bg-card border border-border">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={createGroup} disabled={!groupName.trim() || selectedMembers.length === 0}
              className="flex-1 py-2 rounded-xl bg-primary text-primary-foreground text-sm font-medium disabled:opacity-50">Crear</button>
          </div>
        </div>
      )}
        ...
        conversations.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-4">
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-muted-foreground">Selecciona un amigo para chatear</p>
            </div>
          </div>
        )
      )}

      {/* Tab Amigos - para iniciar chat directo */}
      {tab === 'friends' && (
        friends.length > 0 ? (
          <div className="flex flex-col gap-2">
            {friends.map(f => (
              <button key={f.uid} onClick={() => openDirectChat(f.uid)}
                className="flex items-center gap-3 p-3 rounded-xl bg-card border border-border hover:border-primary/30 transition-all text-left">
                <div className="w-11 h-11 rounded-full bg-secondary flex items-center justify-center font-bold text-muted-foreground overflow-hidden">
                  {f.avatar ? <img src={f.avatar} className="w-full h-full object-cover" /> : (f.name || '?')[0].toUpperCase()}
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">{f.status === 'playing' ? `🎮 ${f.current_game || 'Jugando'}` : f.status === 'scheduled' ? '⏰ Va a jugar' : 'Offline'}</p>
                </div>
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-4">
            ...
            <p className="text-sm text-muted-foreground text-center">Añade amigos para poder chatear</p>
          </div>
        )
      )}
    </div>
